(function ($) {
    $.fn.boxModeling = function (options) {
        ...
        const boxHandlers =
            ...
            '<div class="' + settings.handlerClass + ' resize left-top" style="top: -5px;left: -5px; cursor: nw-resize;"></div>' +
            '<div class="' + settings.handlerClass + ' resize left-mid" style="left: -5px;top: calc(50% - 5px); cursor: w-resize;"></div>' +
            '<div class="' + settings.handlerClass + ' resize left-bot" style="bottom: -5px;left: -5px; cursor: sw-resize;"></div>' +
            '<div class="' + settings.handlerClass + ' resize center-top" style="top: -5px;left: calc(50% - 5px); cursor: n-resize;"></div>' +
            '<div class="' + settings.handlerClass + ' resize center-bot" style="bottom: -5px;left: calc(50% - 5px); cursor: s-resize;"></div>' +
            '<div class="' + settings.handlerClass + ' resize right-top" style="top: -5px;right: -5px; cursor: ne-resize;"></div>' +
            '<div class="' + settings.handlerClass + ' resize right-mid" style="right: -5px;top: calc(50% - 5px); cursor: e-resize;"></div>' +
            '<div class="' + settings.handlerClass + ' resize right-bot" style="bottom: -5px;right: -5px; cursor: se-resize;"></div>' +
            '<div class="' + settings.handlerClass + ' rotate" style="top: -30px;left: calc(50% - 5px); cursor: grab;"></div>' +
            '</div>';
        return $(this).each(function () {
            ...
            function resizeHandler(event, left, top, xResize, yResize) {
                ...
                function eventMoveHandler(event) {
                    ...
                    const fontSize = calculateFontSize(newW, newH);
                    $(box).find('.box').css('font-size', fontSize + 'px');
                }
            ...
            function calculateFontSize(width, height) {
                ...
                const maxHeight = 100; // Set your maximum height
                const maxFontSize = 120; // Set your maximum font size
                const minFontSize = 12; // Set your minimum font size

                let fontSize = maxFontSize;

                while (fontSize > minFontSize && (height > maxHeight || width > maxWidth)) {
                    fontSize--;
                    height = $(box).find('.box').height();
                    width = $(box).find('.box').width();
                }
